...
! Use funtions originally derived from Malinka et al. (2018) to calculate the
! melt pond albedo.
! Malinka et al. (2018). Reflective properties of melt ponds on sea ice. The
! Cryosphere. Volume 12. Issue 6. 1921–1937
! https://doi.org/10.5194/tc-12-1921-2018

IMPLICIT NONE

! Inputs
REAL(KIND=real_jlslsm), INTENT(IN) :: cos_zenith_angle
                                   ! Cos of the zenith angle of incident light
REAL(KIND=real_jlslsm), INTENT(IN) :: pond_depth
                                   ! The depth of the melt pond (m)
REAL(KIND=real_jlslsm), INTENT(IN) :: bottom_albedo(4)
                                   ! The albedo of the sea ice at the base of
                                   ! the melt pond in radiation bands

! Outputs
REAL(KIND=real_jlslsm), INTENT(OUT) :: pond_albedo(4)
                                      ! The melt pond albedos on each band
                                      ! 1 = Direct visible
                                      ! 2 = Diffuse visible
...
! Locals
! Angle information
REAL(KIND=real_jlslsm) :: angle_air
                       ! The angle of the light on the air side of the pond
REAL(KIND=real_jlslsm) :: angle_water
                       ! The angle of the light on the water side of the pond
REAL(KIND=real_jlslsm) :: sin_angle_water
                       ! The SIN of the angle of the light on the water side
REAL(KIND=real_jlslsm) :: cos_angle_water
                       ! The COS of the angle of the light on the water side
REAL(KIND=real_jlslsm) :: cos_angle_air
                       ! The COS of the angle of the light on the air side

! Other variables
REAL(KIND=real_jlslsm) :: reflected_light
         ! The fraction of light reflected off the surface of the melt pond
REAL(KIND=real_jlslsm) :: transmitted_light
         ! The fraction of light transmitted into the melt pond
REAL(KIND=real_jlslsm) :: x
         ! Variable x in the Malinka equations 4 and 5. Equal to the
         ! extinction coefficient multipled by the pond depth.
REAL(KIND=real_jlslsm) :: f_out
         ! The result of the f_out equation (equation 5 of Malinka)
         ! Calculated using a best fit approximation instead of an integral
REAL(KIND=real_jlslsm) :: f_in
         ! The result of the f_in equation (equation 4 of Malinka)
         ! Calculated using a best fit approximation instead of an integral

! Constants
REAL(KIND=real_jlslsm), PARAMETER :: n_air = 1
         ! Refractive index for air
REAL(KIND=real_jlslsm), PARAMETER :: n_water = 1.33
         ! Refractive index of water
REAL(KIND=real_jlslsm), PARAMETER :: ext_coeff_visible = 0.2152
         ! Extintion coeffient of water
         ! from NEMO trc_oce.F90 rkrgb lookup table for 1.0 mg m-3 chlorophyll
         ! (averaged over blue, green and red)
REAL(KIND=real_jlslsm), PARAMETER :: ext_coeff_nir = 2.857
         ! Extintion coeffient of near infrared light in water
         ! 2.857 = 1.0 / rn_si0 = value used by NEMO
REAL(KIND=real_jlslsm), PARAMETER :: rfd = 0.0659
...
! Calculate the sin of transmitted angle (Snell's law)
...
reflected_light = reflected_fresnel(cos_angle_air, cos_angle_water, n_air,     &
                                    n_water)
transmitted_light = 1.0 - reflected_light

! ----------- Section on visible light ------------------

! For visible light calculate integrals in equations 4 and 5 by using
! pre-calculated best fit parameters calculated using
! malinka_find_best_fits.py in:
...
pond_albedo(1) = reflected_light + transmitted_light *                         &
                 EXP(-x/cos_angle_water) * f_out * bottom_albedo(2) /          &
...
! For NIR light calculate integrals in equations 4 and 5 by using
! pre-calculated best fit parameters calculated using
! malinka_find_best_fits.py in:
! https://github.com/MetOffice/malinka_melt_ponds
...
pond_albedo(3) = reflected_light + transmitted_light *                         &
                 EXP(-x/cos_angle_water) * f_out * bottom_albedo(4) /          &
                 ( n_water**2 * (1.0 - bottom_albedo(4) * f_in) )
...
pond_albedo(4) = rfd + f_out**2 * bottom_albedo(4) /                           &
                ( n_water**2 * (1.0 - bottom_albedo(4) * f_in) )
...
FUNCTION reflected_fresnel(cos_angle_in, cos_angle_out, n_in, n_out)           &
                           RESULT(r_f)
...
! Calculate the amount of reflected light off of a interface between
! two fluids using the Fresnel equations
